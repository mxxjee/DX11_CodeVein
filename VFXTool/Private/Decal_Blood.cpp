...
HRESULT Decal_Blood::Ready_Components()
{
	...
	/* For.Com_Texture */
	if (FAILED(__super::Add_Component(_UINT(LEVEL::MAIN), TEXT("Prototype_Component_Texture_Decal"),
		TEXT("Com_Texture"), reinterpret_cast<Component**>(&m_pTextureCom))))
		return E_FAIL;

	/* For.Com_Texture */
	if (FAILED(__super::Add_Component(_UINT(LEVEL::MAIN), TEXT("Prototype_Component_Texture_Decal_Mask"),
...
HRESULT Decal_Blood::Bind_ShaderResources()
{
	...
	// b0: 카메라 버퍼 전체 (View, Proj, InverseView, InverseProj, CamPos 등)
	CHECK_FAILED(m_pGameInstance->Bind_PipeLine_All(m_pShaderCom), E_FAIL);
