...
HRESULT Terrain::Initialize(void* pArg)
{
	...
	m_pTransformCom->Set_State(DIRECTION::POSITION, XMVectorSet(-32.f, -5.f, -12.f, 1.f));
...
{
	//게임오브젝트에서 복사생성
	if (FAILED(__super::Add_Component(_UINT(LEVEL::MAIN), TEXT("Prototype_Component_VIBuffer_Terrain"),
		L"Com_VIBuffer", reinterpret_cast<Component**>(&m_pVIBuffer))))
		return E_FAIL;

	//쉐이더
	if (FAILED(__super::Add_Component(_UINT(LEVEL::STATIC), TEXT("Prototype_Component_Shader_VTXNorTex"),
		L"Com_Shader", reinterpret_cast<Component**>(&m_pShaderCom))))
		return E_FAIL;

	//텍스쳐 만들기
	if (FAILED(__super::Add_Component(_UINT(LEVEL::MAIN), TEXT("Prototype_Component_Texture_Terrain"),
