...
_int SwordTrail::Update(const _float fTimeDelta)
{
    ...
    if (m_bIsTest)
    {
        m_fTestAngle += m_fTestSpeed * fTimeDelta;

        _float fCos = cosf(m_fTestAngle);
        _float fSin = sinf(m_fTestAngle);

        // Root : 회전의 중심
        _float3 vRoot = _float3(fCos * m_fTestRadius * 0.3f, 0.f, fSin * m_fTestRadius * 0.3f);

        // Tip : 회전의 바깥
        _float3 vTip = _float3(fCos * m_fTestRadius, 0.f, fSin * m_fTestRadius);

        Add_TrailPoint(vRoot, vTip);
    }
