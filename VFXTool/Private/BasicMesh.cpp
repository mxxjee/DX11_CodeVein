...
{
	/* For.Com_Model */
	CHECK_FAILED(__super::Add_Component(_UINT(LEVEL::MAIN), L"Prototype_Component_Model_SM_FX_GEZPlate04",
		L"Com_Model", reinterpret_cast<Component**>(&m_pModelCom)), E_FAIL);

	/* For.Com_Shader */
	if (FAILED(__super::Add_Component(_UINT(LEVEL::STATIC), TEXT("Prototype_Component_Shader_MeshEffect"),
		TEXT("Com_Shader"), reinterpret_cast<Component**>(&m_pShaderCom))))
		return E_FAIL;
	m_strShaderName = "Prototype_Component_Shader_MeshEffect";
...
HRESULT BasicMesh::Change_Model(const _wstring strModelTag)
{
	...
	m_pModelCom = static_cast<Model*>(m_pGameInstance->Clone_Prototype(PROTOTYPE::COMPONENT, _UINT(LEVEL::MAIN), strModelTag));
