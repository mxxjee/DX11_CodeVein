...
HRESULT Player_Head::Initialize(void* arg)
{
	...
	cout << "Head Remap Fail Count : " << failCount << " / " << m_BoneRemap.size() << endl;
...
HRESULT Player_Head::Render(const _float fTimeDelta)
{
	...
	vector<Bone*>& MasterRigBones = pMasterRigModelCom->Get_Bones();
...
HRESULT Player_Head::Ready_Components()
{
	...
	CHECK_FAILED(Add_Component(m_iLevel, L"Prototype_Component_Model_PlayerHead_Base", Com_Model, RCAST(Component**)(&m_pModelCom)), E_FAIL);
