...
Texture2D<float4> g_DiffuseTexture : register(t100);
...
PS_OUT PS_MAIN(PS_IN In)
{
    ...
    float4 vDstDiffuse = g_DiffuseTexture.Sample(LinearSampler, In.vTexcoord * 20.f);
    
    //float4 vMask = g_MaskTexture.Sample(LinearSampler, In.vTexcoord);
    //
    //float4 vMtrlDiffuse = vDstDiffuse;
