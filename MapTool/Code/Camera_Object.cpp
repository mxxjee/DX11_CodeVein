...
HRESULT CCamera_Object::Initialize(void* arg)
{
    ...
    m_fMinRotationX = XMConvertToRadians(-10.f);
...
void CCamera_Object::Update_Camera(const _float fTimeDelta)
{
    if (m_bIsActive)
    {
        ...
        _vector vNewPos = XMVectorLerp(vCurrentPos, vCamDest, fTimeDelta * m_fSmoothness);
    ...
    //    _long MouseX = m_pGameInstance->MouseMove(MOUSEMOVESTATE::X);
    //    _long MouseY = m_pGameInstance->MouseMove(MOUSEMOVESTATE::Y);

    //    if (MouseX != 0 || MouseY != 0)
    //    {
    //        m_fAngleX += MouseX * m_fMouseSensitivity * fTimeDelta; // 좌우 회전 (Yaw)
    //        m_fAngleY += MouseY * m_fMouseSensitivity * fTimeDelta; // 상하 회전 (Pitch)
    //    }

    //    // 상하 회전 제한
    //    if (m_fAngleY > XMConvertToRadians(80.0f))
    //        m_fAngleY = XMConvertToRadians(80.0f);
    //    if (m_fAngleY < XMConvertToRadians(-80.0f))
    //        m_fAngleY = XMConvertToRadians(-80.0f);
    ...
    //    _matrix matRot = XMMatrixRotationRollPitchYaw(m_fAngleY, m_fAngleX, 0.f);

    //    _vector vOffset = XMVectorSet(0.f, 0.f, -m_fTargetDistance, 1.f);
