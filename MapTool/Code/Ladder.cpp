...
HRESULT CLadder::Ready_Components()
{
    ...
    CHECK_FAILED(Add_Component(iLevel, L"Prototype_Component_Model_LadderTop", L"Com_Model_Top", RCAST(Component**)(&m_pModelTopCom)), E_FAIL);
    CHECK_FAILED(Add_Component(iLevel, L"Prototype_Component_Model_LadderMiddle", L"Com_Model_Middle", RCAST(Component**)(&m_pModelMiddleCom)), E_FAIL);
    CHECK_FAILED(Add_Component(iLevel, L"Prototype_Component_Model_LadderBottom", L"Com_Model_Bottom", RCAST(Component**)(&m_pModelBottomCom)), E_FAIL);
...
HRESULT CLadder::Render(const _float fDT)
{
    ...
    if (m_pModelTopCom != nullptr)
    {
        ...
        if (FAILED(m_pShaderCom->Bind_RawValue("g_WorldMatrix", &matFinalFloat4x4, sizeof(_float4x4))))
            return E_FAIL;

        for (_uint j = 0; j < m_pModelTopCom->Get_NumMeshes(); j++)
        {
            CHECK_FAILED(m_pModelTopCom->Bind_Material(m_pShaderCom, "g_DiffuseTexture", j, aiTextureType_DIFFUSE), E_FAIL);
            _uint iPass = (m_vecObjPass.size() > j) ? m_vecObjPass[j] : 0;
    ...
    for (_uint i = 0; i < m_iLength; ++i)
    {
        ...
        {
            CHECK_FAILED(pCurrentModel->Bind_Material(m_pShaderCom, "g_DiffuseTexture", j, aiTextureType_DIFFUSE), E_FAIL);
            _uint iPass = (m_vecObjPass.size() > j) ? m_vecObjPass[j] : 0;
            CHECK_FAILED(m_pShaderCom->Begin(iPass), E_FAIL);
            CHECK_FAILED(m_pShaderCom->Bind_Resources(iPass), E_FAIL);
...
CLadder* CLadder::Create(ID3D11Device* pD, ID3D11DeviceContext* pC)
{
