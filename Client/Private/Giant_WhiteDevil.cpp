#include "Client_Define.h"
#include "Giant_WhiteDevil.h"

#include "MonsterState.h"
#include "PhysX_Function.h"

#include "MState_Turn.h"
#include "MWeapon_Halberd.h"
#include "Mstate_Awake_Idle.h"

#include "UIObj_HpBar.h"
#include "UIObj_BossName.h"

#include "InventoryManager.h"

#include "UIObj_BossDead.h"
#include "UIObj_Minimap.h"

#include "TrailEffect.h"

Client::Giant_WhiteDevil::Giant_WhiteDevil(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    :BossBase(pDevice,pContext)
{
}

Client::Giant_WhiteDevil::Giant_WhiteDevil(const Giant_WhiteDevil& original)
    :BossBase(original)
{
}

Client::Giant_WhiteDevil::~Giant_WhiteDevil()
{
}

//////////////////////////////////////////////////////// 객체 준비 함수 ////////////////////////////////////////////////////////
HRESULT Client::Giant_WhiteDevil::Initialize_Prototype(LEVEL _level)
{
    m_iLevel = _UINT(_level);

    return S_OK;
}

HRESULT Client::Giant_WhiteDevil::Initialize(void* _arg)
{
    static _uint namenum = 0;

    MONSTER_DESC desc;

    if (_arg == nullptr)
    {
        _arg = &desc;
    }
    else
    {
        MONSTER_DESC* monsterdesc = CAST(MONSTER_DESC*)(_arg);
    }

    CHECK_FAILED(Ready_Controller(), E_FAIL);

    CAST(MONSTER_DESC*)(_arg)->wstrName = L"Giant_WhiteDevil" + to_wstring(namenum++);

    CHECK_FAILED(Monster::Initialize(_arg), E_FAIL);

    CHECK_FAILED(Ready_Components(), E_FAIL);
    //무기-sword장착
    CHECK_FAILED(Ready_PartObjects(), E_FAIL);

    CHECK_FAILED(Ready_PhysXEvent(), E_FAIL);
    CHECK_FAILED(Ready_Event(), E_FAIL);
    CHECK_FAILED(Ready_States(), E_FAIL);
    CHECK_FAILED(Ready_Value(), E_FAIL); // 스탯 컴포넌트는 여기
    CHECK_FAILED(Ready_Socket(), E_FAIL);


#ifdef _DEBUG
#else
    Base* pointer = this;
    Safe_AddRef(pointer);
#endif // _DEBUG


    // 컨트롤러 위치 맞춰주기

    m_pController->setFootPosition(ToPxExtendedVec3(Get_Position()));

    m_pGameInstance->PhysX_Disable_Controller(m_pController); //컨트롤러 비활성화

    m_fMeshYOffset = 0.f;

    return S_OK;
}
////////////////////////////////////////////////////////

#pragma region 컨테이너추가함수
//////////////////////////////////////////////////////// 컨테이너 추가 함수 ////////////////////////////////////////////////////////
HRESULT Client::Giant_WhiteDevil::Ready_Controller()
{
    // 피직스 세팅
    PHYSX_CONTROLLER_DESC controllerdesc;
    controllerdesc.fRadius = 0.8f;
    controllerdesc.fHeight = 0.95f;
    controllerdesc.fSlopeLimit = 50.f;
    controllerdesc.eActorType = PHYSX_ACTOR_TYPE::MONSTER;
    controllerdesc.iObjectID = m_iObjectID;
    controllerdesc.pOwner = this;

    // 피직스 컨트롤러 만들기
    m_pController = Create_Controller(controllerdesc);

    return S_OK;
}

HRESULT Client::Giant_WhiteDevil::Ready_Components()
{
    __super::Ready_Components();

    CHECK_FAILED(Add_Shader(Proto_Com_Shader_VTXAnimMesh), E_FAIL);	// 쉐이더
    CHECK_FAILED(Add_Model(Proto_Model(L"Giant_WhiteDevil")), E_FAIL);	// 모델
    CHECK_FAILED(Add_StateMachine(), E_FAIL);						// 상태 머신
    CHECK_FAILED(Add_Minimap_Monster(), E_FAIL);					// 미니맵

    m_pModelCom->Set_Animation_CS(1, true);
    m_pModelCom->Play_Animation_CS(0.2f);
    m_pModelCom->Set_Animation_CS(0, true);

    return S_OK;
}

HRESULT Client::Giant_WhiteDevil::Ready_PhysXEvent()
{
    // 충돌처리 해야되니까 모델에x 오브젝트 ID 등록
    m_pModelCom->Set_OwnerId(m_iObjectID);

    // 공격 이벤트 구독(애니메이션이 쏴줌)
    m_iColliderEventHandle = GameObject::Subscribe_Event<COLLIDER_EVENT>(
        [this](const COLLIDER_EVENT& _event)
        {
            if (_event.iOwnerId != m_iObjectID)
                return;

            switch (_event.ePhase)
            {
            case ANIM_FRAMEPHASE::START:
                m_bAttackSweepActive = true;
                m_fAttackRadius = _event.fAttackRadius;
                m_fAttackDamage = _event.fAttackDamage;
                m_fAttackHalfHeight = _event.fAttackHalfHeight;
                m_setHitTargets.clear();
                m_vPrevWeaponPos = m_vCurrentWeaponPos;

                /*    if (m_pModelCom->Get_CurrentAnimationIndex() == 10)
                    m_pWeaponBoneMatrix = const_cast<_float4x4*>(m_pActiveWeapon->Get_Model()->Get_BoneMatrixPtr("IkLeftHandSocket"));

                else
                    m_pWeaponBoneMatrix = const_cast<_float4x4*>(m_pActiveWeapon->Get_Model()->Get_BoneMatrixPtr("TrailEndSocket"));
                */
                break;

            case ANIM_FRAMEPHASE::UPDATE:
                break;

            case ANIM_FRAMEPHASE::END:
                m_bAttackSweepActive = false;
                m_setHitTargets.clear();
                break;
            }
        });

    // 데미지 받는 이벤트
    Subscribe_Event<DAMAGE_EVENT>([this](DAMAGE_EVENT _event) {
        if (_event.iTargetID != m_iObjectID)
            return;

        Monster::OnDamaged(_event);
        });

#pragma region deadui알람
    m_DeadUIApearAlarm.Limit = 2.f;
    m_DeadUIApearAlarm.m_AlarmFunc = [this]()
        {
            /*2초뒤에 켜지고, 5초뒤에 꺼져라*/
            UIObj_BossDead::BossDeadUIEvent Event;
            Event.m_fDeadSecond = 8.f;
            m_pGameInstance->Publish(Event);

            m_DeadUIApearAlarm.Off();

        };

    m_DeadUIApearAlarm.Off();


    return S_OK;
}

HRESULT Client::Giant_WhiteDevil::Ready_Event()
{
    // 몬스터 애니메이션 이벤트 구독
    Subscribe_Event<MONSTER_ANIM_EVENT>([this](const MONSTER_ANIM_EVENT& _event) {
        // 내꺼인지 체크
        if (_event.iOwnerId != m_iObjectID)
            return;

        // 호밍 스피드 및 엑스트라 이동거리 수정
        switch (_event.ePhase)
        {
        case ANIM_FRAMEPHASE::START:
            if (_event.bChangeValue)
            {
                if (_event.bEnableSlide)
                    m_fLungeOffsetSq = _event.fTargetOffset * _event.fTargetOffset;
                if (_event.bEnableHoming)
                {
                    m_fTurnSpeed = _event.fHomingSpeedDegree;
                    m_fLungeDistance = _event.fMaxMoveDistance;
                }
            }
            m_bComboTriggeredThisFrame = _event.bComboAttack;\
            break;

        case ANIM_FRAMEPHASE::UPDATE:
            break;

        case ANIM_FRAMEPHASE::END:
            break;
        }

        });

    // 이펙트 이벤트 구독
    Subscribe_Event<PARTICLE_EVENT>([this](const PARTICLE_EVENT& e) {
        if (e.iOwnerId != m_iObjectID)
            return E_FAIL;

        ParticleSystem* pSystem = CAST(ParticleSystem*)(POOLING->Acquire_Effect(e.ePoolId));
        if (pSystem == nullptr)
            return E_FAIL;

        // 뼈 행렬 계산 람다
        auto fnCalcBoneMatrix = [this, socketName = e.SocketName, bAttached = e.bAttached]() -> _float4x4
            {
                _float4x4 matFinalPos = {};
                const _float4x4* pMatBone = { nullptr };

                if (bAttached)
                {
                    // 무기 뼈
                    pMatBone = m_pActiveWeapon->Get_Model()->Get_BoneMatrixPtr(socketName);
                    //pMatBone = nullptr;
                    if (pMatBone != nullptr)
                    {
                        _matrix matWorld = XMLoadFloat4x4(pMatBone);

                        // 스케일 제거 => TransformCoord 하면서 Range 값이 너무 작아짐 방지
                        matWorld.r[0] = XMVector3Normalize(matWorld.r[0]);
                        matWorld.r[1] = XMVector3Normalize(matWorld.r[1]);
                        matWorld.r[2] = XMVector3Normalize(matWorld.r[2]);

                        XMStoreFloat4x4(&matFinalPos, matWorld);
                    }
                }
                else
                {
                    // 몬스터 뼈
                    pMatBone = m_pModelCom->Get_SocketBoneMatrixPtr_Index(socketName);
                    if (pMatBone != nullptr)
                    {
                        _matrix matBone = XMLoadFloat4x4(pMatBone);
                        _matrix matWorld = matBone * m_pTransformCom->Get_WorldMatrix();

                        // 스케일 제거 => TransformCoord 하면서 Range 값이 너무 작아짐 방지
                        matWorld.r[0] = XMVector3Normalize(matWorld.r[0]);
                        matWorld.r[1] = XMVector3Normalize(matWorld.r[1]);
                        matWorld.r[2] = XMVector3Normalize(matWorld.r[2]);

                        XMStoreFloat4x4(&matFinalPos, matWorld);
                    }
                }

                return matFinalPos;
            };

        // 초기 위치 설정
        _float4x4 matInitial = fnCalcBoneMatrix();
        pSystem->Set_WorldMatrix(matInitial);

        pSystem->Play();

        // Follow 면 콜백에 등록
        if (e.bFollow)
            pSystem->Set_FollowCallback(fnCalcBoneMatrix);

        return S_OK;
        });

    // Dissolve 이벤트 구독
    Subscribe_Event<DISSOLVE_EVENT>([this](const DISSOLVE_EVENT& e) {
        if (e.iOwnerId != m_iObjectID)
            return E_FAIL;

        if (e.bIsWeaponTarget)
            return S_OK;

        m_bDissolving = true;

        if (e.bDissolve)
        {
            // 사라지기
            m_fDissolveDir = 1.f;
            m_fTimeElapsed = 0.f;
        }
        else
        {
            // 나타나기
            m_fDissolveDir = -1.f;
            m_fTimeElapsed = m_fDissolveMax;
        }
        });

    Subscribe_Event<TRAIL_EVENT>([this](const TRAIL_EVENT& e) {
        if (e.iOwnerId != m_iObjectID)
            return E_FAIL;

        if (e.ePhase == ANIM_FRAMEPHASE::START)
        {
            // 2페이즈면 다른 이펙트
            POOL_ID eActualPoolID = e.ePoolId;
            if (m_bPhase2 && e.ePoolId == POOL_ID::EFFECT_ENEMY_SLASH)
                eActualPoolID = POOL_ID::EFFECT_WHITEDEVIL_SLASH;
            
            // 이펙트 매니저에서 ParticleSystem 찾기
            ParticleSystem* pSystem = CAST(ParticleSystem*)(POOLING->Acquire(eActualPoolID));

            if (pSystem != nullptr)
            {
                pSystem->Play();
                m_vecTrailEffects.clear();
                auto vecEffects = pSystem->Get_Effects();
                for (auto& entry : vecEffects)
                {
                    TrailEffect* pTrail = DCAST(TrailEffect*)(entry.pEffect);
                    if (pTrail != nullptr)
                        m_vecTrailEffects.push_back(pTrail);
                }
            }

            m_bTrailActive = true;
        }
        else if (e.ePhase == ANIM_FRAMEPHASE::END)
        {
            m_bTrailActive = false;
            m_vecTrailEffects.clear();
        }

        return S_OK;
        });


    return S_OK;
}

HRESULT Client::Giant_WhiteDevil::Ready_States()
{
    ///AWAKE
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_AWAKE_IDLE, AWAKE_IDLE), E_FAIL);
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_AWAKE, AWAKE), E_FAIL);


    //IDLE  & DEAD
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_IDLE, IDLE), E_FAIL);
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_DEAD, DEATH), E_FAIL);

    
    //Movement
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_RUN, RUN_F, 1.5f), E_FAIL);

    CHECK_FAILED(Add_State(MONSTER_STATE::MS_WALK, WALK_F,1.2f), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_WALK, WALK_F, 1.2f, WALK_DIR::WALK_FRONT), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_WALK, WALK_L, 1.2f, WALK_DIR::WALK_LEFT), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_WALK, WALK_R, 1.2f, WALK_DIR::WALK_RIGHT), E_FAIL);

    //회피
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_EVADE, DODGE_L, 1.2f), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_EVADE, DODGE_L, 1.0f, EAVDE_DIR::EVADE_LEFT), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_EVADE, DODGE_R, 1.0f, EAVDE_DIR::EVADE_RIGHT), E_FAIL);


    //공격
    //Normal Attack
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_ATTACKMELEE0, NOMRAL_MELEE0, 1.0f, DAMAGEPOWER::NORMALPOWER), E_FAIL);
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_ATTACKMELEE1, NOMRAL_MELEE1, 1.0f, DAMAGEPOWER::STRONGPOWER), E_FAIL);//콤보 연계용
  

    //강한공격
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_ATTACKMELEE2, STRONG_MELEE0, 1.0f, DAMAGEPOWER::STRONGPOWER), E_FAIL);
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_ATTACKMELEE3, STRONG_MELEE1, 1.0f, DAMAGEPOWER::STRONGPOWER), E_FAIL);
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_ATTACKMELEE4, STRONG_MELEE2, 1.0f, DAMAGEPOWER::HEAVYPOWER), E_FAIL);


    //먼 공격
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_ATTACKFAR0, SPIN_ATTACK, 1.0f, DAMAGEPOWER::SPECIALPOWER), E_FAIL);

    //스페셜공격
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_PHASECHANGE, SKILL_BUFFSELF, 1.0f, DAMAGEPOWER::SPECIALPOWER), E_FAIL);
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_ATTACKSPECIAL0, SKILL_POINTSHOOT, 1.0f, DAMAGEPOWER::SPECIALPOWER), E_FAIL);

    //TURN
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_TURN, TURN180L, 3.0f), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_TURN, TURN90_L, 4.f, MState_Turn::TurnAnimation::TURN_LEFT), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_TURN, TURN90_R, 4.f, MState_Turn::TurnAnimation::TURN_RIGHT), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_TURN, TURN180L, 4.f, MState_Turn::TurnAnimation::TURN_LEFT180), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_TURN, TURN180R, 4.f, MState_Turn::TurnAnimation::TURN_RIGHT180), E_FAIL);


    CHECK_FAILED(Add_State(MONSTER_STATE::MS_BATTLE, ANIMINDEX_END), E_FAIL);

    //HITSTATE들
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_HIT, DAMAGE_FL), E_FAIL);//일반 anim
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_HIT, DAMAGE_FL, 0.8f, MakeHitKey(HIT_LEVEL::HIT_NORMAL, HIT_DIR::HIT_FL)), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_HIT, DAMAGE_FR, 0.8f, MakeHitKey(HIT_LEVEL::HIT_NORMAL, HIT_DIR::HIT_FR)), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_HIT, DAMAGE_BL, 0.8f, MakeHitKey(HIT_LEVEL::HIT_NORMAL, HIT_DIR::HIT_BL)), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_HIT, DAMAGE_BR, 0.8f, MakeHitKey(HIT_LEVEL::HIT_NORMAL, HIT_DIR::HIT_BR)), E_FAIL);

    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_HIT, DAMAGE_STRONG_FL, 1.0f, MakeHitKey(HIT_LEVEL::HIT_STRONG, HIT_DIR::HIT_FL)), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_HIT, DAMAGE_STRONG_FR, 1.0f, MakeHitKey(HIT_LEVEL::HIT_STRONG, HIT_DIR::HIT_FR)), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_HIT, DAMAGE_STRONG_BL, 1.0f, MakeHitKey(HIT_LEVEL::HIT_STRONG, HIT_DIR::HIT_BL)), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_HIT, DAMAGE_STRONG_BR, 1.0f, MakeHitKey(HIT_LEVEL::HIT_STRONG, HIT_DIR::HIT_BR)), E_FAIL);



    //CHECK_FAILED(Add_State(MONSTER_STATE::MS_HIT, DAMAGE_FL), E_FAIL);//일반 anim


    m_pStateMachineCom->Set_State(MONSTER_STATE::MS_AWAKE_IDLE);
    m_eIdleType = ENEMY_IDLE_TYPE::STATIC;

    CHECK_FAILED(Load_TransitionTable(L"../../DataFiles/Monster_StateTable/Giant_WhiteDevil.jsonc"), E_FAIL);
    return S_OK;

}

HRESULT Client::Giant_WhiteDevil::Ready_PartObjects()
{
    // 무기 부착용 소켓 데이터(component로딩이후 호출)
    CHECK_FAILED(m_pModelCom->Register_SocketBoneName("RightHandAttach"), E_FAIL);
    CHECK_FAILED(m_pModelCom->Register_SocketBoneName("Spine"), E_FAIL);

    //m_pWeaponBoneMatrix = m_pModelCom->Get_SocketBoneMatrixPtr(m_pModelCom->Get_SocketBoneIndex("RightHandAttach"));

    MWeapon_Halberd::WEAPONHALBERD_DESC SwordDesc{};
    SwordDesc.bIsDead = false;
    SwordDesc.pMasterRig = this;
    SwordDesc.iSocketIndex = m_pModelCom->Get_SocketBoneIndex("RightHandAttach");
    SwordDesc.pSocketMatrix = m_pModelCom->Get_SocketBoneMatrixPtr_Index("RightHandAttach");
    SwordDesc.pParentMatrix = m_pTransformCom->Get_WorldMatrixPtr();
    SwordDesc.wstrModelName = Proto_Model(L"WhiteLargeHalberd");
    SwordDesc.eWeaponType = WEAPON_TYPE::SWORD;
    SwordDesc.vWeaponScale = _float3(1.1f, 1.2f, 1.1f);
    SwordDesc.bIdentity = true;
    SwordDesc.pDissolveTime = &m_fTimeElapsed;
    SwordDesc.pDissolveMax = &m_fDissolveMax;

    CHECK_FAILED(__super::Add_PartObject(m_iLevel,
        Proto_GameObject(L"Monster_Weapon_Sword"),
        L"Part_Sword",
        &SwordDesc), E_FAIL);


    m_pWeapons[_UINT(WEAPON_TYPE::SWORD)] = dynamic_cast<Monster_Weapon*>(Find_PartObject(TEXT("Part_Sword")));
    if (m_pWeapons[_UINT(WEAPON_TYPE::SWORD)] == nullptr)
        return E_FAIL;

    Safe_AddRef(m_pWeapons[_UINT(WEAPON_TYPE::SWORD)]);

    m_pActiveWeapon = m_pWeapons[_UINT(WEAPON_TYPE::SWORD)];
    m_pActiveWeapon->Set_PartActive(true);

    //끝~끝 
    //m_pWeaponBoneMatrix = const_cast<_float4x4*>(m_pActiveWeapon->Get_Model()->Get_BoneMatrixPtr("TrailEndSocket"));	// 콜라이더용
    m_pWeaponTrailRoot = const_cast<_float4x4*>(m_pActiveWeapon->Get_Model()->Get_BoneMatrixPtr("TrailEndSocket"));
    m_pWeaponTrailTip = const_cast<_float4x4*>(m_pActiveWeapon->Get_Model()->Get_BoneMatrixPtr("TrailStartSocket"));
    m_pWeaponBoneMatrix = const_cast<_float4x4*>(m_pActiveWeapon->Get_Model()->Get_BoneMatrixPtr("Root"));
    m_pWeaponBoneMatrixEnd = const_cast<_float4x4*>(m_pActiveWeapon->Get_Model()->Get_BoneMatrixPtr("TrailStartSocket"));



    return S_OK;
}

HRESULT Client::Giant_WhiteDevil::Ready_Value()
{
    m_pTransformCom->Set_Scale(1.0f, 1.0f, 1.0f);

    m_bIsSquared = false;

    /* 스탯 */
#ifdef _DEBUG
    m_fMaxHp = 14000.f;/// 8500.f;
#else
    m_fMaxHp = 14000.f;/// 8500.f;
#endif
    m_fCurrentHp = m_fMaxHp;

    /* 이동 및 회전 속도 */
    m_fWalkSpeed = 3.f;             // 걷기 최대 속도
    m_fWalkAcceleration = 5.f;      // 걷기 가속도
    m_fRunSpeed = 5.f;              // 달리기 최대 속도
    m_fRunAcceleration = 9.f;       // 달리기 가속도
    m_fNormalRotationSpeed = 60.f;  // RotateSpeed 평상시
    m_fChaseRotationSpeed = 180.f;    // RotateSpeed 추격시
    m_fTurnSpeed = 0.f;             // 턴 인터럽트 시 회전 속도
    m_fTurnTimeLimit = 0.f;         // 턴 인터럽트 최대 허용 시간
    m_eTurnType = TURN_TYPE::TURN_END; // 플레이어쪽으로 방향 전환 타입

    //★SPecail 스킬을 사용할 수 있는 거리 설정하기 - ★//

    m_fEvadeSpeed = 1.f;
    m_fEvadeAccelration = 1.7f;
    /* 거리 및 범위 */
    m_fWalkRange = 10.f;             // 어디쯤부터 간 보면서 걸을지
    m_fDetectRange = 15.0f;         // IDLE상태일때 어디까지 탐지할지
    m_fChaseRange = 50.0f;          // BATTLE상태일 때 어디까지 쫒아갈지
    m_fAttackMeleeRange = 5.f;      // 근접공격 시작 범위
    m_fAttackFarRange = 7.f;        // 원거리공격 범위(0이면 없음)
    m_fAttackDuration = 2.f;		// 공격 딜레이

    /* 시야각 */
    m_fFOV = XMConvertToRadians(110.f);		// FOV값 설정(시야각) *필요하면 수정해야함 수치조절*
    m_bIgnoreFOV = false;					// FOV 무시

    /* 정찰관련 */
    m_vPatrolTargetPos;			// 뭐 줘야할지 아직은 모르겠다
    m_fPatrolRange = 10.f;      // 정찰반경

    /* 방어 및 상태 (슈아/그로기) */
    m_fMaxGrit = 14.f;              // 강인도 최대치
    m_fCurrentGrit = m_fMaxGrit;
    m_bSuppressDown = true;			// 다운/뒤잡기 면역
    m_bIsSuperArmor = false;        // 기본값 false, 공격 애니메이션 시 true로 제어
    m_bBackAttackMonster = false;
    m_bCanParry = false;


    /*HIT 목표값*/
    m_fTargetCumulativeDamage = 1000.f;
    m_fMaxCumulativeTime = 1.f;
    m_bIgnoreNormalHit = true;


    /* 스탯 컴포넌트 추가 */
    Add_Stat_Monster(m_fMaxHp, m_fAttackDamage, m_iDefence, 10000);

    //UI에 띄울 이름
    m_ApperUIName_Kr = L"백랑의 광전사";
    m_ApperUIName_Eng = L"Argent Wolf Berserker";

    // YOffset 변경용 알람
    m_YOffsetAlarm.m_AlarmFunc = [this] {
        m_bOffsetChanged = false;
        m_YOffsetAlarm.Stop();
        };

    m_YOffsetAlarm.Limit = 0.2f;

    // Dissolve 시간
    m_fDissolveMax = 3.f;

    return S_OK;
}

HRESULT Client::Giant_WhiteDevil::Ready_Socket()
{
    // 매 프레임 갱신할 무기 소켓(얘는 손에 달려있음)
    //CHECK_FAILED(m_pModelCom->Register_SocketBoneName("RightHandAttach"), E_FAIL);
    CHECK_FAILED(m_pModelCom->Register_SocketBoneName("Spine"), E_FAIL);
    // 포인터로 가져와서 값 항상 동일하게 만들기
    // m_pWeaponBoneMatrix = m_pModelCom->Get_SocketBoneMatrixPtr_Index("RightHand");
    //HECK_FAILED(m_pModelCom->Register_SocketBoneName("LeftHand_Ketsugi_Socket"), E_FAIL);


    return S_OK;
}
#pragma endregion


//////////////////////////////////////////////////////// 최초 업데이트 함수 ////////////////////////////////////////////////////////
_int Client::Giant_WhiteDevil::Update_Priority(const _float fTimeDelta)
{
    CHECK_FUNCTION_FAIL(__super::Update_Priority(fTimeDelta));

    // Priority에서 애니메이션 업데이트
    m_pModelCom->Play_Animation_CS(fTimeDelta);

    return 0;
}
/*******************************************************/

_int Client::Giant_WhiteDevil::Update_Parallel(const _float fTimeDelta)
{

    CHECK_FUNCTION_FAIL(__super::Update_Parallel(fTimeDelta));

    if (m_pGameInstance->KeyDown(DIK_LEFTARROW))
    {
        m_fCurrentHp -= 500.f;
    }

    m_pModelCom->Update_Socket_Simple();
    XMStoreFloat3(&m_vRootMotionDelta, Calculate_RootPos(true, true, true));

    // 무기 위치 업데이트
    Update_WeaponPosition_Capsule();

    // 루트모션값 받아와서 저장

    // 플레이어쪽으로 바라보기
    LookAt_Smooth_Target(fTimeDelta);

    // 이동속도 결정
    Update_Speed(fTimeDelta);


    // 무기가 있으면
    if (m_pActiveWeapon == nullptr)
        return -1;

    

    return 0;
}



_int Client::Giant_WhiteDevil::Update(const _float fTimeDelta)
{
    CHECK_FUNCTION_FAIL(__super::Update(fTimeDelta));

    // 상태 전이 테이블 확인
    Update_Transition();

    // 상태머신 컴포넌트 업데이트
    if (m_pStateMachineCom)
        m_pStateMachineCom->Update_State(fTimeDelta);

    if (m_bIsKilled)
    {
        m_pStateMachineCom->Change_State(Monster::MS_DEAD);
    }

    Set_Offset(fTimeDelta);

    // 이 동
    Move_Smooth(fTimeDelta);

    // 공격중이라면 공격상태
    Process_AttackSweep_Capsule();

    Update_Trail();

    OnAwarePlayer();

    return 0;
}

_int Client::Giant_WhiteDevil::Update_Late(const _float fTimeDelta)
{
    CHECK_FUNCTION_FAIL(__super::Update_Late(fTimeDelta));
    Set_Visible_MonsterUI(m_fChaseRange);
    Add_RenderGroup(RENDER_GROUP::NONBLEND);
    Add_RenderGroup(RENDER_GROUP::SHADOW);

#ifdef _DEBUG
    Add_Debug_Controller_Capsule(fTimeDelta);
    Add_Debug_LookLine();

    if (m_bAttackSweepActive)
    {
        Add_Debug_WeaponCapsule(m_fAttackRadius);
    }
#endif // _DEBUG

    return 0;
}

HRESULT Client::Giant_WhiteDevil::Render(const _float fTimeDelta)
{
    CHECK_FALSE_RESULT(m_bIsActive, E_FAIL);
    CHECK_FALSE_RESULT(m_bIsVisible, E_FAIL);

    CHECK_FAILED(__super::Render(fTimeDelta), E_FAIL);

    //MSG_FAIL(Bind_ShaderResources(), (m_wstrName + L"\n객체의 바인딩 과정중 오류가 발생했습니다.").c_str(), L"렌더 실패", E_FAIL);

    CHECK_FAILED(m_pShaderCom->Begin(12), E_FAIL);

    for (_uint i = 0; i < m_pModelCom->Get_NumMeshes(); i++)
    {
        _uint bitFlag = 0;
        CHECK_FAILED(m_pModelCom->Bind_Material_FullSlot(m_pShaderCom, 0, i, aiTextureType_DIFFUSE, 0, &bitFlag), E_FAIL);
        CHECK_FAILED(m_pModelCom->Bind_Material_FullSlot(m_pShaderCom, 1, i, aiTextureType_NORMALS, 0, &bitFlag), E_FAIL);
        CHECK_FAILED(m_pModelCom->Bind_Material_FullSlot(m_pShaderCom, 4, i, aiTextureType_METALNESS, 0, &bitFlag), E_FAIL);
        CHECK_FAILED(m_pModelCom->Bind_Material_FullSlot(m_pShaderCom, 10, i, aiTextureType_EMISSIVE, 0, &bitFlag), E_FAIL);

        m_pShaderCom->Bind_RawValue_ByHandle(g_bitFlag, &bitFlag, sizeof(_uint));

        m_pModelCom->Bind_CombiendAndOffsetSRV(m_pShaderCom);
        CHECK_FAILED(m_pModelCom->Bind_ReampSRV(m_pShaderCom, i), E_FAIL);

        CHECK_FAILED(m_pShaderCom->Bind_Resources(12), E_FAIL);
        CHECK_FAILED(m_pModelCom->Render(fTimeDelta, i), E_FAIL);
    }

    return S_OK;
}

HRESULT Client::Giant_WhiteDevil::Render_Shadow(const _float fTimeDelta, _int iCascadeNum)
{
    // Noise Texture
    _uint bUseNoise = (m_fTimeElapsed > 0.f) ? 1 : 0;
    CHECK_FAILED(m_pShaderCom->Bind_RawValue_ByHandle(g_bUseNoise, &bUseNoise, sizeof(_uint)), E_FAIL);
    if (bUseNoise)
    {
        _float fNormalized = m_fTimeElapsed / m_fDissolveMax;
        CHECK_FAILED(m_pNoiseTextureCom->Bind_ShaderResource_FullSlot(m_pShaderCom, 12, 0), E_FAIL);
        m_pShaderCom->Bind_RawValue_ByHandle(g_fTimeElapsed, &fNormalized, sizeof(_float));
    }

    CHECK_FAILED(m_pTransformCom->Bind_WorldMatrix_ByHandle(m_pShaderCom, g_WorldMatrix), E_FAIL);

    CHECK_FAILED(m_pShaderCom->Begin(4, false), E_FAIL);

    for (size_t i = 0; i < m_pModelCom->Get_NumMeshes(); i++)
    {
        CHECK_FAILED(m_pModelCom->Bind_CombiendAndOffsetSRV(m_pShaderCom), E_FAIL);

        CHECK_FAILED(m_pModelCom->Bind_ReampSRV(m_pShaderCom, i), E_FAIL);

        CHECK_FAILED(m_pShaderCom->Bind_Resources(4), E_FAIL);

        CHECK_FAILED(m_pModelCom->Render(fTimeDelta, i), E_FAIL);
    }
    return S_OK;
}

//////////////////////////////////////////////////////// 바인딩 함수 ////////////////////////////////////////////////////////
HRESULT Client::Giant_WhiteDevil::Bind_ShaderResources()
{
    m_pTransformCom->Bind_WorldMatrix_ByHandle(m_pShaderCom, g_WorldMatrix);

    return S_OK;
}
/******************************************************* 바인딩 함수 *******************************************************/



//////////////////////////////////////////////////////// YOffset 변경 함수 ////////////////////////////////////////////////////////
void Client::Giant_WhiteDevil::Set_Offset(const _float fTimeDelta)
{
    _uint animIndex = m_pModelCom->Get_CurrentAnimationIndex();

    // 이 전 애니메이션 인덱스와 현재 애니메이션 인덱스가 다르면
    if (animIndex != m_iPrevAnimIndex)
    {
        _float newOffset = 0.f;
        switch (animIndex)
        {
        case STRONG_MELEE0: case STRONG_MELEE1: case STRONG_MELEE2:
        case DEATH:
            newOffset = 0.6f;
            break;
        default:
            newOffset = 0.f;
            break;
        }

        if (newOffset != m_fCurrentYOffset)
        {
            m_fPrevYOffset = m_fMeshYOffset;
            m_fCurrentYOffset = newOffset;
            m_bOffsetChanged = true;
            m_YOffsetAlarm.Elapsed = 0.f;
            m_YOffsetAlarm.On();
        }

        m_iPrevAnimIndex = animIndex;
    }

    if (m_bOffsetChanged)
    {
        m_YOffsetAlarm.Update(fTimeDelta);
        _float ratio = clamp(m_YOffsetAlarm.Elapsed / m_YOffsetAlarm.Limit, 0.f, 1.f);
        m_fMeshYOffset = fLerp(m_fPrevYOffset, m_fCurrentYOffset, ratio);
    }

}

void Client::Giant_WhiteDevil::Change_Phase()
{
    m_bPhase2 = true;
    m_pStateMachineCom->Find_State(MS_ATTACKMELEE0)->Set_AnimSpeed(1.2f);
    m_pStateMachineCom->Find_State(MS_ATTACKMELEE1)->Set_AnimSpeed(1.2f);
    m_pStateMachineCom->Find_State(MS_ATTACKMELEE2)->Set_AnimSpeed(1.2f);
    m_pStateMachineCom->Find_State(MS_ATTACKMELEE3)->Set_AnimSpeed(1.2f);
    m_pStateMachineCom->Find_State(MS_ATTACKMELEE4)->Set_AnimSpeed(1.2f);
    m_pStateMachineCom->Find_State(MS_ATTACKFAR0)->Set_AnimSpeed(1.2f);
}
/******************************************************* YOffset 변경 함수 *******************************************************/

void Client::Giant_WhiteDevil::OnAwarePlayer()
{
    CHECK_JUST_NULL(m_pStateMachineCom);

    _uint	iCurState = m_pStateMachineCom->Get_CurStateType();
    CHECK_FALSE(iCurState == MS_AWAKE);

    if (!m_bFirstAware)
    {
        //////////////////////////////////HP바연동///////////
        UIObject* pBossHpBar = m_pGameInstance->Find_UI_ByName(L"Boss_HPBar");
        if (pBossHpBar)
        {
            UIObj_HpBar* pProgressBar = dynamic_cast<UIObj_HpBar*>(pBossHpBar);
            if (pProgressBar)
            {
                pProgressBar->Set_CurrentFloat(Get_CurrentHp());
                pProgressBar->Set_MaxFloat(Get_MaxHp());

                pProgressBar->Init_Ratio(true, 1.f);
                pProgressBar->Init_Ratio(false, 1.f);


                pBossHpBar->Set_Active(false, false);

                pProgressBar->Set_Owner_ObjectID(Get_ObjectID());
                pProgressBar->Set_NameTex("Textures/WhiteDevil_Name");
            }
        }

        ///////////////////////////////////////////////////
        //이름켜주기!!내부에서 알아서 보스hp바랑연동.
        UIObj_BossName::BossNameUIEVent BossNameEvent;
        BossNameEvent.bActive = true;
        BossNameEvent.BossName_Korean = m_ApperUIName_Kr;
        BossNameEvent.BossName_English = m_ApperUIName_Eng;
        BossNameEvent.English_Spacing = 20.f;
        BossNameEvent.English_LocalPos = _float2(-22.f, 0.f);
        BossNameEvent.m_eBossType = BOSS_TYPE::WHITE_DEVIL;

        m_pGameInstance->Publish(BossNameEvent);

        m_pGameInstance->Play_Sound("Battle_Boss_BGM", 0.15f, true);
        m_bFirstAware = true;

        ///미니맵끄기
        UIObject* pMinimap = m_pGameInstance->Find_UI_ByName(L"Minimap_Center");
        if (pMinimap)
            pMinimap->Set_Visible(false);
    }
}


//////////////////////////////////////////////////////// Pooling 함수 ////////////////////////////////////////////////////////
void Client::Giant_WhiteDevil::OnSpawn(void* _arg)
{
    //__super::OnSpawn(_arg);

    m_bIsActive = true;
    m_bIsKilled = false;
    m_bEnablePhysics = true;
    m_fDeadTimer = 0.f;
    m_iTotalDamage = 0;
    m_fCurrentHp = m_fMaxHp;
    m_bFirstAware = false;

    m_pStateMachineCom->Set_State(MONSTER_STATE::MS_AWAKE_IDLE);

    Set_CinematicState(Monster::CINEMATIC_END);
    Set_StunState(Monster::STUNSTATE_END);

    if (_arg == nullptr)
    {

    }
    else
    {
        GameObject::GAMEOBJECT_DESC* desc = CAST(GAMEOBJECT_DESC*)(_arg);

        m_pTransformCom->Set_State(DIRECTION::POSITION, desc->vPosition);
        Rotation(XMConvertToRadians(desc->vRotation.x), XMConvertToRadians(desc->vRotation.y), XMConvertToRadians(desc->vRotation.z));

        m_pGameInstance->PhysX_Enable_Controller(m_pController, ToPxExtendedVec3(XMLoadFloat4(&desc->vPosition)), PX_ACTOR_TYPE::MONSTER);
        m_vSpawnPosition.x = desc->vPosition.x;
        m_vSpawnPosition.y = desc->vPosition.y;
        m_vSpawnPosition.z = desc->vPosition.z;
    }

    m_PreStatusVisible = VISIBLE_STATE::VS_NONE;


    // 무기 부활
    if (m_umapPartObjects.size() > 0)
    {
        for (auto& part : m_umapPartObjects)
        {
            part.second->Set_ElapsedTime(0.f);
            part.second->Set_OwnerDead(false);
        }
    }

    Calculate_To_PlayerDistance_And_Direction();

    //HRESULT hr = Setup_MonsterStatusBar(_float3(0.f, 3.f, 0.f));
}

void Client::Giant_WhiteDevil::OnDespawn()
{
    __super::OnDespawn();



}

void Client::Giant_WhiteDevil::Update_Trail()
{
    // Trail 재생
    if (m_bTrailActive &&
        m_vecTrailEffects.empty() == false)
    {
        _float3 vFinalRoot = {}, vFinalTip = {};

        vFinalRoot.x = m_pWeaponTrailRoot->_41;
        vFinalRoot.y = m_pWeaponTrailRoot->_42;
        vFinalRoot.z = m_pWeaponTrailRoot->_43;

        vFinalTip.x = m_pWeaponTrailTip->_41;
        vFinalTip.y = m_pWeaponTrailTip->_42;
        vFinalTip.z = m_pWeaponTrailTip->_43;

        for (auto& pTrail : m_vecTrailEffects)
            pTrail->Add_TrailPoint(vFinalRoot, vFinalTip);
    }
}

void   Client::Giant_WhiteDevil::OnDead()
{
    __super::OnDead();
    m_pGameInstance->FadeOutSound("Battle_Boss_BGM", 5.f);
    InventoryManager::GetInstance()->Get_Skill_AfterWhite();

    POOLING->Clear(POOL_ID::EFFECT_WHITEDEVIL_AURA);
}
/******************************************************* Pooling 함수 *******************************************************/

//////////////////////////////////////////////////////// 생성자 호출 함수 ////////////////////////////////////////////////////////
Giant_WhiteDevil* Client::Giant_WhiteDevil::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, LEVEL _level)
{
    Giant_WhiteDevil* pInstance = new Giant_WhiteDevil(pDevice, pContext);

    MSG_FAIL(pInstance->Initialize_Prototype(_level), L"Giant_WhiteDevil 원본 생성 실패", L"경고!!!", nullptr);

    return pInstance;
}


GameObject* Client::Giant_WhiteDevil::Clone(void* pArg)
{
    Giant_WhiteDevil* pInstance = new Giant_WhiteDevil(*this);

    MSG_FAIL(pInstance->Initialize(pArg), L"Giant_WhiteDevil 복사 실패", L"경고!!!", nullptr);

    return pInstance;
}
/******************************************************* 생성자 호출 함수 *******************************************************/

//////////////////////////////////////////////////////// 객체 반환 함수 ////////////////////////////////////////////////////////
void Client::Giant_WhiteDevil::Free()
{
    __super::Free();
}
/******************************************************* 객체 반환 함수 *******************************************************/
