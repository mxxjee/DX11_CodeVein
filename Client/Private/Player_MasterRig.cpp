...
const _float4x4* Player_MasterRig::Get_SocketMatrix(_int _boneNum)
{
	//return m_pModelCom->Get_ReadbackBoneMatrixPtr(pBoneName);
	return m_pModelCom->Get_SocketBoneMatrixPtr(_boneNum);
}

_int Client::Player_MasterRig::Get_SocketIndex(const _string& _boneName)
{
	return m_pModelCom->Get_SocketBoneIndex(_boneName);
}

void Client::Player_MasterRig::Set_Animation(_uint iAnimationIndex, _bool bIsLoop, _float fLerpDuration, _float fAnimationSpeed)
{
...
void Client::Player_MasterRig::Set_AnimationUpper(_uint iAnimationIndex, _bool bUpperBlendEnable, _float fUpperLayerWeight, _bool _isLoop, _float fLerpDuration, _float fAnimationSpeed)
{
...
void Client::Player_MasterRig::Clear_AnimationUpper()
{
	m_pModelCom->Clear_AnimationUpper();
}

void Client::Player_MasterRig::Request_ClearAnimationUpper(_float fFadeOutTime)
{
	m_pModelCom->Request_ClearAnimationUpper(fFadeOutTime);
}

void Client::Player_MasterRig::Sync_BaseToUpperAnim()
{
	m_pModelCom->Sync_BaseToUpperAnim();
}
...
HRESULT Player_MasterRig::Initialize(void* arg)
{
	...
	CHECK_FAILED(m_pModelCom->Register_SocketBoneName("LeftHand"), E_FAIL);
...
_int Player_MasterRig::Update(const _float fTimeDelta)
{
	...
	_vector qRotationDelta = m_pModelCom->Get_RootMotionRotationDelta();

	m_pPlayerTransform->Rotate_ByQuaternionDelta(qRotationDelta);
