...
Client::MWeapon_Sword::MWeapon_Sword()
{
}

Client::MWeapon_Sword::MWeapon_Sword(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:Monster_Weapon(pDevice, pContext)
{
}

Client::MWeapon_Sword::MWeapon_Sword(const MWeapon_Sword& original)
...
HRESULT Client::MWeapon_Sword::Initialize_Prototype(LEVEL _level)
{
...
HRESULT Client::MWeapon_Sword::Initialize(void* arg)
{
	...
	(pDesc)->wstrName = L"Monster_Weapon_Sword" + to_wstring(namenum++);
...
_int Client::MWeapon_Sword::Update_Priority(const _float fTimeDelta)
{
...
_int Client::MWeapon_Sword::Update_Parallel(const _float fTimeDelta)
{
...
_int Client::MWeapon_Sword::Update(const _float fTimeDelta)
{
...
_int Client::MWeapon_Sword::Update_Late(const _float fTimeDelta)
{
...
HRESULT Client::MWeapon_Sword::Render(const _float fTimeDelta)
{
...
HRESULT Client::MWeapon_Sword::Render_Shadow(const _float fTimeDelta, _int iCascadeNum)
{
	// Noise Texture
...
HRESULT Client::MWeapon_Sword::Ready_Components(void* pArg)
{
	//CHECK_NULL_RESULT(pArg,E_FAIL);

	WEAPONSWORD_DESC* pDesc = static_cast<WEAPONSWORD_DESC*>(pArg);

	CHECK_FAILED(Add_Shader(Proto_Shader(L"VTXMesh")), E_FAIL);
	//CHECK_FAILED(Add_Model(Proto_Model(pDesc->wstrModelName), E_FAIL);

	CHECK_FAILED(Add_Model(pDesc->wstrModelName), E_FAIL);
...
HRESULT Client::MWeapon_Sword::Bind_ShaderResources()
{
	CHECK_FAILED(m_pTransformCom->Bind_WorldMatrix(m_pShaderCom), E_FAIL);
...
MWeapon_Sword* Client::MWeapon_Sword::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, LEVEL _level)
{
	MWeapon_Sword* pInstance = new MWeapon_Sword(pDevice, pContext);

	MSG_FAIL(pInstance->Initialize_Prototype(_level), L"MWeapon_Sword 원본 생성 실패", L"경고!!!", nullptr);

	return pInstance;
}


GameObject* Client::MWeapon_Sword::Clone(void* pArg)
{
	MWeapon_Sword* pInstance = new MWeapon_Sword(*this);

	MSG_FAIL(pInstance->Initialize(pArg), L"MWeapon_Sword 복사 실패", L"경고!!!", nullptr);

	return pInstance;
}

void Client::MWeapon_Sword::Free()
{
	__super::Free();
}
