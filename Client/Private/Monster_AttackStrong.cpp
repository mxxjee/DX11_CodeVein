#include "Client_Define.h"
#include "Monster_AttackStrong.h"
#include "StateMachine.h"
#include "Monster.h"
#include "Monster_Weapon.h"

Client::Monster_AttackStrong::Monster_AttackStrong()
{
}

HRESULT Client::Monster_AttackStrong::Initialize(GameObject* pOwner, _uint iAnimIdx, _float fSpeed)
{
	__super::Initialize(pOwner, iAnimIdx, fSpeed);

	m_pMonsterTransformCom = m_pOwner->Get_Transform();
	m_pStateMachine = dynamic_cast<StateMachine*>(m_pOwner->Get_Component_FromName(Com_StateMachine));
	Set_DamagePower(DAMAGEPOWER::STRONGPOWER);
	m_pMonster = dynamic_cast<Monster*>(m_pOwner);
	Set_HasAnimEvents(true);
	if (m_pMonsterTransformCom == nullptr || m_pStateMachine == nullptr || m_pMonster == nullptr)
		return E_FAIL;

	return S_OK;
}

void Client::Monster_AttackStrong::Enter_State()
{
	if (m_pMonster == nullptr)
		return;

	m_pMonster->Get_Model()->Set_Animation_CS(m_iAnimIdx, false, 0.2f, m_fAnimSpeed);
	m_pMonster->Set_IsAttacking(true);

	//COUT("MonsterState::ATTACKSTRONG");
}

void Client::Monster_AttackStrong::Update_State(_float fTimeDelta)
{
	if (m_pMonster == nullptr) return;

	if (m_pMonster->Get_Model()->Is_AnimFinished())
	{
		m_pStateMachine->Change_State(Monster::IDLE);
	}
}

void Client::Monster_AttackStrong::Exit_State()
{
	m_pMonster->Set_IsAttacking(false);

}

Monster_AttackStrong* Client::Monster_AttackStrong::Create(GameObject* pOwner, _uint iAnimIdx, _float fSpeed)
{
	Monster_AttackStrong* pInstance = new Monster_AttackStrong();

	if (FAILED(pInstance->Initialize(pOwner, iAnimIdx, fSpeed)))
	{
		MSG_BOX("Failed to Created : Monster_AttackStrong");
		Safe_Release(pInstance);
	}

	return pInstance;
}

void Client::Monster_AttackStrong::Free()
{
	__super::Free();
}
