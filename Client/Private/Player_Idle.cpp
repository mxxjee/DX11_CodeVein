...
Client::Player_Idle::Player_Idle()
{
}

HRESULT Client::Player_Idle::Initialize(GameObject* pOwner, Player_MasterRig* pPlayer_MasterRig)
{
...
void Client::Player_Idle::Enter_State()
{
...
void Client::Player_Idle::Update_State(_float fTimeDelta)
{
	Player::INPUT_DESC& Desc = m_pPlayer->Get_PlayerInputDesc();
	

	if (m_pPlayer->CanUseSkill())
	{
		return;
	}
	else if (Desc.bCDown && !m_pPlayer->Get_OnGuard() && !Desc.bZeroStamina)
	{
		m_pStateMachine->Change_State(Player::ATTACKPARRY);
		return;
	}
	else if (Desc.bShiftPressed && (Desc.eLastMouseKeyState == MOUSEKEYSTATE::LB || Desc.eLastMouseKeyState == MOUSEKEYSTATE::RB) && !m_pPlayer->Get_OnGuard() && !Desc.bZeroStamina)
	{
...
void Client::Player_Idle::Exit_State()
{
}

Player_Idle* Client::Player_Idle::Create(GameObject* pOwner, Player_MasterRig* pPlayer_MasterRig)
{
