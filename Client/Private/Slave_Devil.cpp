...
Client::Slave_Devil::Slave_Devil(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : Monster(pDevice, pContext)
{
}

Client::Slave_Devil::Slave_Devil(const Slave_Devil& original)
    : Monster(original)
{
}

HRESULT Slave_Devil::Initialize_Prototype(LEVEL _level)
{
...
HRESULT Slave_Devil::Initialize(void* _arg)
{
    ...
    CAST(MONSTER_DESC*)(_arg)->wstrName = L"Slave_Devil" + to_wstring(namenum++);
   ...
   /* {
    ...
    m_eIdleType = ENEMY_IDLE_TYPE::PATROL;
    ...
    m_vEmissionUV = { 0.1f, 0.2f };

    return S_OK;
}
#pragma region 컨테이너추가함수
//////////////////////////////////////////////////////// 컨테이너 추가 함수 ////////////////////////////////////////////////////////
HRESULT Client::Slave_Devil::Ready_Controller()
{
    // 피직스 세팅
    PHYSX_CONTROLLER_DESC controllerdesc;
    controllerdesc.fRadius = 1.2f;
    controllerdesc.fHeight = 0.95f;
    controllerdesc.fSlopeLimit = 35.f;
...
HRESULT Client::Slave_Devil::Ready_Components()
{
    __super::Ready_Components();

    CHECK_FAILED(Add_Shader(Proto_Com_Shader_VTXAnimMesh), E_FAIL);
    CHECK_FAILED(Add_Model(Proto_Model(L"SlaveDevil")), E_FAIL);
...
HRESULT Client::Slave_Devil::Ready_PhysXEvent()
{
...
HRESULT Client::Slave_Devil::Ready_States()
{
    ...
    //SKILL-DEFENCE
    //쩜공
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_ATTACKFAR0, JUMP_ATTACK, 1.0f, DAMAGEPOWER::SPECIALPOWER), E_FAIL);

    _float AttackSpeed = 1.0f;
    //Nomral1 -> Normal2 / Normal1->Normal3
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_ATTACKMELEE0, NORMAL1, AttackSpeed, DAMAGEPOWER::NORMALPOWER), E_FAIL);
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_ATTACKMELEE1, NORMAL2, AttackSpeed, DAMAGEPOWER::NORMALPOWER), E_FAIL);
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_ATTACKMELEE2, NORMAL3, AttackSpeed, DAMAGEPOWER::NORMALPOWER), E_FAIL);

    CHECK_FAILED(Add_State(MONSTER_STATE::MS_ATTACKSPECIAL0, SPECIAL_MELEE, AttackSpeed, DAMAGEPOWER::STRONGPOWER), E_FAIL);


    /*이동*/
    _float TurnAnimSpeed = 1.5f;
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_TURN, ANIMINDEX_END, TurnAnimSpeed), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_TURN, TURN90_L, TurnAnimSpeed, MState_Turn::TurnAnimation::TURN_LEFT), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_TURN, TURN90_R, TurnAnimSpeed, MState_Turn::TurnAnimation::TURN_RIGHT), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_TURN, TURN180L, TurnAnimSpeed, MState_Turn::TurnAnimation::TURN_LEFT180), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_TURN, TURN180R, TurnAnimSpeed, MState_Turn::TurnAnimation::TURN_RIGHT180), E_FAIL);


    _float WalkAnimSpeed = 1.3f;
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_RUN, RUN_F, 1.0f), E_FAIL);

    CHECK_FAILED(Add_State(MONSTER_STATE::MS_WALK, WALK_F), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_WALK, WALK_L, WalkAnimSpeed, WALK_DIR::WALK_LEFT), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_WALK, WALK_R, WalkAnimSpeed, WALK_DIR::WALK_RIGHT), E_FAIL);
    ...
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_HIT, DAMAGE_FL, 1.f, MakeHitKey(HIT_LEVEL::HIT_NORMAL, HIT_DIR::HIT_FL)), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_HIT, DAMAGE_FR, 1.f, MakeHitKey(HIT_LEVEL::HIT_NORMAL, HIT_DIR::HIT_FR)), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_HIT, DAMAGE_BL, 1.f, MakeHitKey(HIT_LEVEL::HIT_NORMAL, HIT_DIR::HIT_BL)), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_HIT, DAMAGE_BR, 1.f, MakeHitKey(HIT_LEVEL::HIT_NORMAL, HIT_DIR::HIT_BR)), E_FAIL);
    ...
    CHECK_FAILED(Load_TransitionTable(L"../../DataFiles/Monster_StateTable/Slave_Devil.jsonc"), E_FAIL);
    return S_OK;
}

HRESULT Client::Slave_Devil::Ready_PartObjects()
{
    ...
    m_pWeaponBoneMatrix = m_pModelCom->Get_SocketBoneMatrixPtr(m_pModelCom->Get_SocketBoneIndex("RightHandAttachSocket"));

    MWeapon_Sword::WEAPONSWORD_DESC SwordDesc{};
    SwordDesc.bIsDead = false;
    SwordDesc.pMasterRig = this;
    SwordDesc.iSocketIndex = m_pModelCom->Get_SocketBoneIndex("RightHandAttachSocket");
    SwordDesc.pSocketMatrix = m_pModelCom->Get_SocketBoneMatrixPtr_Index("RightHandAttachSocket");
    SwordDesc.pParentMatrix = m_pTransformCom->Get_WorldMatrixPtr();
    SwordDesc.wstrModelName = L"Prototype_Component_Model_MonsterWeapon_SlaveSword";
    SwordDesc.eWeaponType = WEAPON_TYPE::SWORD;
    SwordDesc.vWeaponScale = _float3(1.3f, 1.3f, 1.3f);
...
HRESULT Client::Slave_Devil::Ready_Value()
{
    m_pTransformCom->Set_Scale(1.5f, 1.5f, 1.5f);

    /* 스탯 */
    m_fMaxHp = 3000.0f;             // 정해줘............ 한 30대?
    m_fCurrentHp = m_fMaxHp;


    // 1. 거리 및 범위
    m_fWalkRange = 5.0f;          // AI 발견 거리
    m_fDetectRange = 10.f;          // AI 발견 거리
    m_fChaseRange = 15.0f;           // 추격 포기 거리
    m_fAttackMeleeRange = 6.5f;      // 근접공격 시작 범위
    m_fAttackFarRange = 8.f;        // 원거리공격 범위(0이면 없음)
    ...
    m_fChaseRotationSpeed = 120.f;    // RotateSpeed 추격시
    ...
    m_fFOV = XMConvertToRadians(100.f);		// FOV값 설정(시야각) *필요하면 수정해야함 수치조절*
    m_bIgnoreFOV = false;

    //m_bBackAttackMonster = false;	// 백어택 가능한 몬스터 X
    //4.정찰
    m_fPatrolRange = 10.f;
    ...
    /*HIT 목표값*/
    m_fTargetCumulativeDamage = 1200.f;
    m_fMaxCumulativeTime = 1.f;
    m_bIgnoreNormalHit = false;


    Add_Stat_Monster(m_fMaxHp, _UINT(m_fAttackDamage), m_iDefence, 700);
...
HRESULT Client::Slave_Devil::Ready_Events()
{
    // 몬스터 애니메이션 이벤트 구독
    ...
    Subscribe_Event<PARTICLE_EVENT>([this](const PARTICLE_EVENT& e) {
        ...
        ParticleSystem* pSystem = CAST(ParticleSystem*)(POOLING->Acquire(e.ePoolId, nullptr));
        if (pSystem == nullptr)
            return E_FAIL;
    ...
    Subscribe_Event<TRAIL_EVENT>([this](const TRAIL_EVENT& e) {
        ...
        if (e.ePhase == ANIM_FRAMEPHASE::START)
        {
            ...
            ParticleSystem* pSystem = CAST(ParticleSystem*)(POOLING->Acquire(e.ePoolId));
...
//////////////////////////////////////////////////////// 최초 업데이트 함수 ////////////////////////////////////////////////////////
_int Client::Slave_Devil::Update_Priority(const _float fTimeDelta)
{
...
//////////////////////////////////////////////////////// 업데이트 함수 ////////////////////////////////////////////////////////
_int Client::Slave_Devil::Update_Parallel(const _float fTimeDelta)
{
...
_int Client::Slave_Devil::Update(const _float fTimeDelta)
{
...
_int Client::Slave_Devil::Update_Late(const _float fTimeDelta)
{
...
HRESULT Client::Slave_Devil::Render(const _float fTimeDelta)
{
...
HRESULT Client::Slave_Devil::Render_Shadow(const _float fTimeDelta, _int iCascadeNum)
{
    // Noise Texture
...
//////////////////////////////////////////////////////// Pooling 함수 ////////////////////////////////////////////////////////
// 디버깅 하려고 override 뺀것
void Client::Slave_Devil::OnSpawn(void* _arg)
{
    __super::OnSpawn(_arg);

    //풀에서 꺼낼떄마다 체력바 갱신
    HRESULT hr = Setup_MonsterStatusBar(_float3(0.f, 4.f, 0.f));

}

void Client::Slave_Devil::OnDespawn()
{
    __super::OnDespawn();

}
void Client::Slave_Devil::Update_Trail()
{
    // Trail 재생
    if (m_bTrailActive &&
...
Slave_Devil* Client::Slave_Devil::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, LEVEL _level)
{
    Slave_Devil* pInstance = new Slave_Devil(pDevice, pContext);

    MSG_FAIL(pInstance->Initialize_Prototype(_level), L"Slave_Devil 원본 생성 실패", L"경고!!!", nullptr);

    return pInstance;
}

GameObject* Client::Slave_Devil::Clone(void* pArg)
{
    Slave_Devil* pInstance = new Slave_Devil(*this);

    MSG_FAIL(pInstance->Initialize(pArg), L"Slave_Devil 복사 실패", L"경고!!!", nullptr);

    return pInstance;
}


void Client::Slave_Devil::Free()
{
    __super::Free();
