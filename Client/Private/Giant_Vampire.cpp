...
Client::Giant_Vampire::Giant_Vampire(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    :Monster(pDevice,pContext)
{
}

Client::Giant_Vampire::Giant_Vampire(const Giant_Vampire& original)
    :Monster(original)
{
}

Client::Giant_Vampire::~Giant_Vampire()
{
}
/******************************************************* 생성자, 소멸자 *******************************************************/\

//////////////////////////////////////////////////////// 객체 준비 함수 ////////////////////////////////////////////////////////
HRESULT Client::Giant_Vampire::Initialize_Prototype(LEVEL _level)
{
...
HRESULT Client::Giant_Vampire::Initialize(void* _arg)
{
    ...
    CAST(MONSTER_DESC*)(_arg)->wstrName = L"Giant_Vampire_" + to_wstring(namenum++);
...
HRESULT Client::Giant_Vampire::Ready_Components()
{
    __super::Ready_Components();

    CHECK_FAILED(Add_Shader(Proto_Com_Shader_VTXAnimMesh), E_FAIL);	// 쉐이더
    CHECK_FAILED(Add_Model(Proto_Model(L"GiantVampire")), E_FAIL);	// 모델
...
HRESULT Client::Giant_Vampire::Ready_PhysXEvent()
{
...
HRESULT Client::Giant_Vampire::Ready_Event()
{
    // 몬스터 애니메이션 이벤트 구독
    Subscribe_Event<MONSTER_ANIM_EVENT>([this](const MONSTER_ANIM_EVENT& _event) {
        ...
        {
        case ANIM_FRAMEPHASE::START:
            m_fTurnSpeed = _event.fHomingSpeedDegree;       // 공격시 회전 속도
            m_fLungeDistance = _event.fMaxMoveDistance;     // 공격시 추가 이동 속도
            m_fLungeOffsetSq = _event.fTargetOffset * _event.fTargetOffset; // 타겟과의 거리가 이정도 안에 들어오면 추가 이동 X
            break;
    ...
    Subscribe_Event<PARTICLE_EVENT>([this](const PARTICLE_EVENT& e) {
        ...
        if (e.ePoolId == POOL_ID::EFFECT_ENEMY_SHIELD_RED)
        {
            m_pShieldEffect = pSystem;
        }
...
HRESULT Client::Giant_Vampire::Ready_States()
{
    ...
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_ATTACKMELEE2, MELEE2, 1.0f, DAMAGEPOWER::NORMALPOWER), E_FAIL);
    
    //쩜공
    CHECK_FAILED(Add_State(MONSTER_STATE::MS_ATTACKMELEE3, MELEE3, 1.0f, DAMAGEPOWER::NORMALPOWER), E_FAIL);



    CHECK_FAILED(Add_State(MONSTER_STATE::MS_TURN, ANIMINDEX_END, 1.0f), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_TURN, TURN90_L, 1.3f, MState_Turn::TurnAnimation::TURN_LEFT), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_TURN, TURN90_R, 1.3f, MState_Turn::TurnAnimation::TURN_RIGHT), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_TURN, TURN180L, 1.3f, MState_Turn::TurnAnimation::TURN_LEFT180), E_FAIL);
    CHECK_FAILED(Add_Additional_Animation(MONSTER_STATE::MS_TURN, TURN180R, 1.3f, MState_Turn::TurnAnimation::TURN_RIGHT180), E_FAIL);
    ...
    CHECK_FAILED(Load_TransitionTable(L"../../DataFiles/Monster_StateTable/Giant_Vampire.jsonc"), E_FAIL);
    return S_OK;

}

HRESULT Client::Giant_Vampire::Ready_PartObjects()
{
    ...
    SwordDesc.wstrModelName = L"Prototype_Component_Model_MonsterWeapon_MilitaryLargeHalberd";
    SwordDesc.eWeaponType = WEAPON_TYPE::SWORD;
    SwordDesc.vWeaponScale = _float3(1.4f, 1.4f, 1.4f);
...
HRESULT Client::Giant_Vampire::Ready_Value()
{
    m_pTransformCom->Set_Scale(1.3f, 1.3f, 1.3f);

    m_bIsSquared = false;
    m_bBackAttackMonster = true;
    ...
    m_fDetectRange = 10.0f;         // IDLE상태일때 어디까지 탐지할지
    m_fChaseRange = 35.0f;          // BATTLE상태일 때 어디까지 쫒아갈지
    m_fAttackMeleeRange = 7.f;      // 근접공격 시작 범위
    m_fAttackFarRange =13.f;        // 원거리공격 범위(0이면 없음)
    m_fAttackDuration = 1.f;		// 공격 딜레이

    /* 시야각 */
    m_fFOV = XMConvertToRadians(120.f);		// FOV값 설정(시야각) *필요하면 수정해야함 수치조절*
    ...
    /*HIT 목표값*/
    m_fTargetCumulativeDamage = 500.f;
    m_fMaxCumulativeTime = 2.f;
    m_bIgnoreNormalHit = false;


    /* 스탯 컴포넌트 추가 */
    Add_Stat_Monster(m_fMaxHp, m_fAttackDamage, m_iDefence, 4700);
...
HRESULT Client::Giant_Vampire::Ready_Socket()
{
...
//////////////////////////////////////////////////////// 최초 업데이트 함수 ////////////////////////////////////////////////////////
_int Client::Giant_Vampire::Update_Priority(const _float fTimeDelta)
{
...
_int Client::Giant_Vampire::Update_Parallel(const _float fTimeDelta)
{
...
_int Client::Giant_Vampire::Update(const _float fTimeDelta)
{
...
_int Client::Giant_Vampire::Update_Late(const _float fTimeDelta)
{
...
//////////////////////////////////////////////////////// 렌더 함수 ////////////////////////////////////////////////////////
HRESULT Client::Giant_Vampire::Render(const _float fTimeDelta)
{
    ...
    CHECK_FAILED(m_pShaderCom->Begin(11), E_FAIL);
    ...
    for (_uint i = 0; i < m_pModelCom->Get_NumMeshes(); i++)
    {
        ...
        CHECK_FAILED(m_pShaderCom->Bind_Resources(11), E_FAIL);
...
HRESULT Client::Giant_Vampire::Render_Shadow(const _float fTimeDelta, _int iCascadeNum)
{
    // Noise Texture
...
//////////////////////////////////////////////////////// 바인딩 함수 ////////////////////////////////////////////////////////
HRESULT Client::Giant_Vampire::Bind_ShaderResources()
{
...
void Client::Giant_Vampire::Update_Trail()
{
    // Trail 재생
    if (m_bTrailActive &&
...
//////////////////////////////////////////////////////// Pooling 함수 ////////////////////////////////////////////////////////
void Client::Giant_Vampire::OnSpawn(void* _arg)
{
    __super::OnSpawn(_arg);
    //HRESULT hr = Setup_MonsterStatusBar(_float3(0.f, 3.f, 0.f));
}
void Client::Giant_Vampire::OnDespawn()
{
    __super::OnDespawn();
}

void Client::Giant_Vampire::OnDamaged(const DAMAGE_EVENT& _damageEvent)
{
    __super::OnDamaged(_damageEvent);

    if (m_fCurrentHp <= 0.f || m_bIsKilled)
    {
        if (m_pShieldEffect != nullptr)
        {
            m_pShieldEffect->Stop();
            m_pShieldEffect->Set_Active(false);
            m_pShieldEffect = nullptr;
        }
    }
}
/******************************************************* Pooling 함수 *******************************************************/
...
//////////////////////////////////////////////////////// 생성자 호출 함수 ////////////////////////////////////////////////////////
Giant_Vampire* Client::Giant_Vampire::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, LEVEL _level)
{
    Giant_Vampire* pInstance = new Giant_Vampire(pDevice, pContext);

    MSG_FAIL(pInstance->Initialize_Prototype(_level), L"Giant_Vampire 원본 생성 실패", L"경고!!!", nullptr);

    return pInstance;
}


GameObject* Client::Giant_Vampire::Clone(void* pArg)
{
    Giant_Vampire* pInstance = new Giant_Vampire(*this);

    MSG_FAIL(pInstance->Initialize(pArg), L"Giant_Vampire 복사 실패", L"경고!!!", nullptr);

    return pInstance;
}
/******************************************************* 생성자 호출 함수 *******************************************************/

HRESULT Client::Giant_Vampire::Set_Animation()
{
    return E_NOTIMPL;
}
...
//////////////////////////////////////////////////////// 객체 반환 함수 ////////////////////////////////////////////////////////
void Client::Giant_Vampire::Free()
{
    __super::Free();
}
/******************************************************* 객체 반환 함수 *******************************************************/
