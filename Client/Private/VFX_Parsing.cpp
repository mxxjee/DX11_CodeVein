...
HRESULT VFX_Parsing::Initialize(_uint _level)
{
    m_iLevel = _level;
    ...
    m_iEventHandle=m_pGameInstance->Subscribe<LOADEVENT>([this](const LOADEVENT& e)
        {
...
void VFX_Parsing::Load_EffectDatas()
{
    ...
    for (const auto& systemInfo : m_vecSystemInfo)
    {
        ...
        m_pGameInstance->Add_GameObject_To_Layer(
            _UINT(LEVEL::STATIC), L"Prototype_GameObject_ParticleSystem",
            m_iLevel, L"Layer_Effect", &pGameObject, &systemDesc);
        ...
        for (const auto& effectInfo : systemInfo.vecEffects)
        {
            ...
            if (effectInfo.eEffectType == EFFECTTYPE::EFFECT_PARTICLE)            // 이펙트 타입이 BasicParticle일 경우
            {
                ...
                pEffect = CAST(BasicParticle*)(m_pGameInstance->Clone_Prototype(PROTOTYPE::GAMEOBJECT, _UINT(LEVEL::STATIC),
            ...
            {
                // SwordTrail 객체 먼저 클론
                pEffect = CAST(BasicTrail*)(m_pGameInstance->Clone_Prototype(PROTOTYPE::GAMEOBJECT, _UINT(LEVEL::STATIC),
                ...
                // TRAIL 정보 업데이트
                CAST(BasicTrail*)(pEffect)->Set_TrailDesc(trailDesc);

                CAST(BasicTrail*)(pEffect)->Change_Texture(stringToWstring(effectInfo.strTextureName));
                CAST(BasicTrail*)(pEffect)->Change_NoiseTexture(stringToWstring(effectInfo.strNoiseTextureName));
                CAST(BasicTrail*)(pEffect)->Change_MaskTexture(stringToWstring(effectInfo.strMaskTextureName));
                CAST(BasicTrail*)(pEffect)->Change_Shader(stringToWstring(effectInfo.strShaderTag));
            }
            else if (effectInfo.eEffectType == EFFECTTYPE::EFFECT_MESH)         // 이펙트 타입이 MeshEffect 일 경우
            {
                // BasicMesh 객체 먼저 클론
                pEffect = CAST(BasicMesh*)(m_pGameInstance->Clone_Prototype(PROTOTYPE::GAMEOBJECT, _UINT(LEVEL::STATIC),
        ...
        // 마지막으로 ParticleSystem을 추가 => 자체 컨테이너에 보관하는 방식에서 이펙트 매니저에 저장하는 방식으로 변경
        m_pGameInstance->Add_Effect(pSystem->Get_Name(), pSystem);
    }
}
...
VFX_Parsing* VFX_Parsing::Create(_uint _level)
{
    VFX_Parsing* pInstance = new VFX_Parsing();

    MSG_FAIL(pInstance->Initialize(_level), L"VFX_Parsing 생성 실패", L"경고!!!", nullptr);
...
{
    __super::Free();
    m_pGameInstance->UnsubScribe(m_iEventHandle);
