...
Client::MWeapon_GreatSword::MWeapon_GreatSword()
{
}

Client::MWeapon_GreatSword::MWeapon_GreatSword(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:Monster_Weapon(pDevice, pContext)
{
}

Client::MWeapon_GreatSword::MWeapon_GreatSword(const MWeapon_GreatSword& original)
...
HRESULT Client::MWeapon_GreatSword::Initialize_Prototype(LEVEL _level)
{
...
HRESULT Client::MWeapon_GreatSword::Initialize(void* arg)
{
	...
	(pDesc)->wstrName = L"Monster_Weapon_GreatSword" + to_wstring(namenum++);
...
_int Client::MWeapon_GreatSword::Update_Priority(const _float fTimeDelta)
{
...
_int Client::MWeapon_GreatSword::Update_Parallel(const _float fTimeDelta)
{
	CHECK_FUNCTION_FAIL(__super::Update_Parallel(fTimeDelta));
	return 0;
}

_int Client::MWeapon_GreatSword::Update(const _float fTimeDelta)
{
	CHECK_FUNCTION_FAIL(__super::Update(fTimeDelta));
	return 0;
}

_int Client::MWeapon_GreatSword::Update_Late(const _float fTimeDelta)
{
...
HRESULT Client::MWeapon_GreatSword::Render(const _float fTimeDelta)
{
...
HRESULT Client::MWeapon_GreatSword::Render_Shadow(const _float fTimeDelta, _int iCascadeNum)
{
	// Noise Texture
...
HRESULT Client::MWeapon_GreatSword::Ready_Components()
{
...
HRESULT Client::MWeapon_GreatSword::Bind_ShaderResources()
{
	CHECK_FAILED(m_pTransformCom->Bind_WorldMatrix(m_pShaderCom), E_FAIL);
...
MWeapon_GreatSword* Client::MWeapon_GreatSword::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, LEVEL _level)
{
	MWeapon_GreatSword* pInstance = new MWeapon_GreatSword(pDevice, pContext);

	MSG_FAIL(pInstance->Initialize_Prototype(_level), L"MWeapon_GreatSword 원본 생성 실패", L"경고!!!", nullptr);

	return pInstance;
}


GameObject* Client::MWeapon_GreatSword::Clone(void* pArg)
{
	MWeapon_GreatSword* pInstance = new MWeapon_GreatSword(*this);

	MSG_FAIL(pInstance->Initialize(pArg), L"MWeapon_GreatSword 복사 실패", L"경고!!!", nullptr);

	return pInstance;
}

void Client::MWeapon_GreatSword::Free()
{
	__super::Free();
}
