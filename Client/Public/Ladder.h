...
class CLadder final : public MapObject, IInteractable
{
public:
    enum class LadderMode{LADDERUP,LADDERDOWN,END};
private:
...
public:
    static CLadder* Create(ID3D11Device* pD, ID3D11DeviceContext* pC, LEVEL _level);
    virtual GameObject* Clone(void* arg) override;
    virtual void Free() override;

private:
    void        Show_LadderUp_UI();
    void        Show_LadderDown_UI();


#pragma region IInteractable
public:
    virtual bool		IsInteractable();	//상호작용가능한지 물어보는 함수
    virtual _float		Get_InteractionRange() { return 1.f; }         //
...
protected:
    ...
    LadderMode  m_eLadderMode = LadderMode::END;
