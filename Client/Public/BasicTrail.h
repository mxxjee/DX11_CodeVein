#pragma once
#include "Client_Define.h"
#include "TrailEffect.h"

NS_BEGIN(Engine)
class VIBuffer_Trail;
class Shader;
class NewTexture;
NS_END

NS_BEGIN(Client)

class BasicTrail final : public TrailEffect
{
private:
	explicit BasicTrail(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	explicit BasicTrail(const BasicTrail& original);
	virtual ~BasicTrail() = default;

public:
	_float& Get_DistortionStrength() { return m_fDistortionStrength; }

	void	Set_DistortionStrength(_float fDistortionStrength) { m_fDistortionStrength = fDistortionStrength; }

public:
	virtual HRESULT Initialize_Prototype() override;
	virtual HRESULT Initialize(void* pArg) override;
	virtual _int    Update_Priority(const _float fTimeDelta) override;
	virtual _int    Update(const _float fTimeDelta) override;
	virtual _int    Update_Late(const _float fTimeDelta) override;
	virtual HRESULT Render(const _float fTimeDelta) override;

public:
	virtual HRESULT Ready_Components() override;
	virtual HRESULT Bind_ShaderResources() override;

public:
	HRESULT Change_Texture(const _wstring strFilePath);								// 텍스쳐 교체 함수
	HRESULT Change_NoiseTexture(const _wstring strFilePath);						// 노이즈 텍스쳐 교체 함수
	HRESULT Change_MaskTexture(const _wstring strFilePath);							// 마스크 텍스쳐 교체 함수
	HRESULT Change_Shader(const _wstring strShaderTag);								// 쉐이더 교체 함수

private:
	_float m_fAccumulatedTime = {};

public:
	static BasicTrail* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	GameObject* Clone(void* pArg) override;
	virtual void Free() override;
};

NS_END