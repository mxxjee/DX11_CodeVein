                     ...
                     _In_ int       nCmdShow)
{
    ...
    MainAnimation* pMainAnimation = MainAnimation::Create();
    MSG_NULL(pMainAnimation, L"MainAnimation이 nullptr입니다!", L"경고!!!", 0);
    ...
    while (true)
    {
        ...
        if (TargetFPS == 0.f)
        {
            pMainAnimation->Update(TimeDelta);
            pMainAnimation->Render(TimeDelta);
        }
        else
        {
            Timer += TimeDelta;
            if (Timer >= 1.f / TargetFPS)
            {
                ...
                pMainAnimation->Update(TimeDeltaTargetFPS);
                pMainAnimation->Render(TimeDeltaTargetFPS);
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.lpszMenuName   = NULL; //MAKEINTRESOURCEW(IDC_ANIMATIONTOOL);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    lstrcpy(szTitle, L"AnimationTool");
