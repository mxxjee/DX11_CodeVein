...
HRESULT BasicParticle::Render(const _float fTimeDelta)
{
	...
	m_pGameInstance->Set_DepthStencilState(DSSSET::DEPTH);
...
{   
	/* For.Com_VIBuffer */
	if (FAILED(__super::Add_Component(_UINT(LEVEL::MAIN), TEXT("Prototype_Component_VIBuffer_Particle_Basic"),
		TEXT("Com_VIBuffer"), reinterpret_cast<Component**>(&m_pVIBufferCom))))
		return E_FAIL;

	/* For.Com_ComputeShader */
	if (FAILED(__super::Add_Component(_UINT(LEVEL::STATIC), TEXT("Prototype_Component_Shader_Compute_Particle"),
		TEXT("Com_ComputeShader"), reinterpret_cast<Component**>(&m_pComputeShaderCom))))
		return E_FAIL;
	m_strComputeShaderName = "Prototype_Component_Shader_Compute_Particle";

	/* For.Com_Shader */
	if (FAILED(__super::Add_Component(_UINT(LEVEL::STATIC), TEXT("Prototype_Component_Shader_Particle_Rect"),
		TEXT("Com_Shader"), reinterpret_cast<Component**>(&m_pShaderCom))))
		return E_FAIL;
	m_strShaderName = "Prototype_Component_Shader_Particle_Rect";
...
HRESULT BasicParticle::Bind_ShaderResources()
{
	...
	CHECK_FAILED(m_pTransformCom->Bind_ShaderResource_FullSlot(m_pShaderCom, BUFFER_EFFECT, "g_EffectWorldMatrix"), E_FAIL);
