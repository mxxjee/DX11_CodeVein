...
private:
    /* 그래픽 디바이스의 초기화. */
    /* 장치객체를 생성한다. */
    HRESULT Initialize(HWND _hWnd, WINMODE _isWindowed, _uint _wincx, _uint _wincy, ID3D11Device** ppDevice, ID3D11DeviceContext** ppContext);

public:
    /* 백버퍼를 지운다. */
    HRESULT Clear_BackBufferView(const _float4* pClearColor);

    /* 깊이버퍼 + 스텐실버퍼를 지운다. */
    HRESULT Clear_DepthStencilView();


    /* 후면 버퍼를 전면버퍼로 전환한다.(백버퍼를 화면에 직접 보여준다.) */
    HRESULT Present();

    /* 화면 크기 변경 */
    HRESULT ResizeBuffers(_int _width, _int _height, D3D11_VIEWPORT& _vp);
...
private:
    ...
    HRESULT Ready_SwapChain(HWND _hWnd, WINMODE _isWindowed, _uint _wincx, _uint _wincy);
    HRESULT Ready_BackBufferRenderTargetView();
    HRESULT	Ready_DepthStencilView(_uint _wincx, _uint _wincy);

public:
    // 원본 화면
    inline ID3D11RenderTargetView*  Get_BackBufferRTV()		const { return m_pBackBufferRTV; }
    inline ID3D11DepthStencilView*  Get_DepthStencilView()	const { return m_pDepthStencilView; }

    // [제거됨: Picking 관련 접근자 전체 삭제]
...
private:
    ...
    /* 1. 메모리 할당. (정점버퍼, 인덱스버퍼, 텍스쳐로드, 쉐이더객체를 생성한다. ) 컴객체의 생성과 관련된 역할 */
...
public:
    static Graphic_Device* Create(HWND _hWnd, WINMODE _isWindowed, _uint _wincx, _uint _wincy, ID3D11Device** ppDevice, ID3D11DeviceContext** ppContext);
