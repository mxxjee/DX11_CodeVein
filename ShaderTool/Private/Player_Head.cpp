...
HRESULT Player_Head::Render(const _float fTimeDelta)
{
	...
	for (_uint i = 0; i < m_pModelCom->Get_NumMeshes(); i++)
	{
		...
		else //얼굴
		{
			...
			//CHECK_FAILED(m_pModelCom->Bind_Material_FullSlot(m_pShaderCom, 7, i, aiTextureType_SHININESS, 0, &test), E_FAIL);
			//CHECK_FAILED(m_pModelCom->Bind_Material_FullSlot(m_pShaderCom, 1, i, aiTextureType_OPACITY, 0, &test), E_FAIL);
			CHECK_FAILED(m_pModelCom->Bind_Material_FullSlot(m_pShaderCom, 4, i, aiTextureType_METALNESS, 0, &bitflag), E_FAIL);
		...
		// 1 g_bitFlag
		m_pShaderCom->Bind_RawValue_FullSlot(1, "g_bitFlag", &bitflag, sizeof(_uint));
...
HRESULT Player_Head::Ready_EyeShaderValue()
{
	...
	m_tPlayerEyeShaderDesc.fPlayerTattoScale = 0.75;  //타투 크기
	m_tPlayerEyeShaderDesc.fPlayerTattoOffsetU = 0.1f; //타투 X위치
	m_tPlayerEyeShaderDesc.fPlayerTattoOffsetV = 0.2f; //타투 y위치
