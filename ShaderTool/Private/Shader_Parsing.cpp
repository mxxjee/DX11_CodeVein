...
//////////////////////////////////////////////////////// 생성자, 소멸자 ////////////////////////////////////////////////////////
ShaderTool::Shader_parsing::Shader_parsing()
{
}
...
//////////////////////////////////////////////////////// 초기화 함수 ////////////////////////////////////////////////////////
HRESULT ShaderTool::Shader_parsing::Initialize()
{
...
//////////////////////////////////////////////////////// 저장 함수 ////////////////////////////////////////////////////////
void ShaderTool::Shader_parsing::Start_Save(const SAVEEVENT& _event)
{
    COUT("[Shader_parsing] 저장 시작");
    ...
    if (_event.bSaveJson)
    {
        ...
        COUT("[Shader_parsing] JSON 저장 완료");
    }

    if (_event.bSaveBinary)
    {
        // TODO: Binary 저장 구현 후 활성화
        COUT("[Shader_parsing] Binary 저장 (미구현)");
    }

    COUT("[Shader_parsing] 저장 완료 - " + to_string(vecData.size()) + "개 쉐이더 설정");
}

void ShaderTool::Shader_parsing::Collect_Data()
{
    vecData.clear();
    ...
    for (const auto& layerName : vecTargetLayers)
    {
        ...
        for (auto& [tag, pObj] : mapObjects)
        {
            ...
            //여기서 모델 찾고 비었으면 하위있는지 확인
            Model* pModel = pObj->Get_Model();
            //컨테이너라면 아래 파츠들 돌면서 추출한다
            OBJTYPE Type = pObj->Get_OBJType();
            if (Type == OBJTYPE::TYPE_CONTAINER)
            {
                unordered_map<_wstring, PartObject*> PartObjs = CAST(ContainerObject*)(pObj)->Get_PartObjectsMap();
                for (auto& Part : PartObjs)
                {
                    pModel = Part.second->Get_Model();
                    if (!pModel)
                        continue;
                    // ObjectKey 추출
                    ...
                    info.iObjectType = _UINT(Part.second->Get_OBJType());

                    // 쉐이더 프로토타입명 가져오기
                    Shader* pShader = Part.second->Get_Shader(); //각 클래스(아래)에서 구현하기
                    if (pShader)
                    {
                    ...
                    // 패스 설정
                    info.vecMeshPasses = Part.second->Get_Passes();
            ...
            if (!pModel || Type == OBJTYPE::TYPE_CONTAINER)
                continue;
...
//////////////////////////////////////////////////////// 로드 함수 ////////////////////////////////////////////////////////
void ShaderTool::Shader_parsing::Start_Load(const LOADEVENT& _event)
{
    COUT("[Shader_parsing] 로드 시작");

    if (m_pGameInstance->LoadFromJson("../../DataFiles/Level_Sample/ShaderData.json", *this))
    {
        COUT("[Shader_parsing] JSON 로드 성공 - " + to_string(vecData.size()) + "개 쉐이더 설정");
    ...
    {
        COUT("[Shader_parsing] JSON 로드 실패!");
    }

    COUT("[Shader_parsing] 로드 완료");
}
/******************************************************* 로드 함수 *******************************************************/
...
//////////////////////////////////////////////////////// 생성자 호출 함수 ////////////////////////////////////////////////////////
Shader_parsing* ShaderTool::Shader_parsing::Create()
{
    Shader_parsing* pInstance = new Shader_parsing();
...
//////////////////////////////////////////////////////// 객체 반환 함수 ////////////////////////////////////////////////////////
void ShaderTool::Shader_parsing::Free()
{
    __super::Free();
}
/******************************************************* 객체 반환 함수 *******************************************************/