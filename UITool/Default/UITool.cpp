...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance,
    ...
    // 전역 문자열을 초기화합니다.
    lstrcpy(szWindowClass, L"UITool_Window_Class");
    lstrcpy(szTitle, L"UITool");
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_UITOOL));
    MSG msg{};
    msg.message = NULL;

    MainUITool* MainUITool = MainUITool::Create();
    MSG_NULL(MainUITool, L"MainUITool이 nullptr입니다!", L"경고!!!", 0);
    ...
    while (true)
    {
        ...
        if (TargetFPS == 0.f)
        {
            MainUITool->Update(TimeDelta);
            MainUITool->Render(TimeDelta);
        }
        else
        {
            Timer += TimeDelta;
            if (Timer >= 1.f / TargetFPS)
            {
                ...
                MainUITool->Update(TimeDeltaTargetFPS);
                MainUITool->Render(TimeDeltaTargetFPS);
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_UITOOL));
