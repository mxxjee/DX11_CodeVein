...
void PS_MAIN_CASCADE_SHADOW(PS_IN_SHADOW In)
{
    ...
    if (g_bUseNoise)
    {
        float fLifeRatio = saturate(g_fTimeElapsed / 2.f);
...
PS_OUT PS_MAIN_NORMAL_PBR(PS_IN_NORMAL In)
{
    ...
    {
        vMtrlDiffuse = float4(0.208, 0.092, 0.04f, 1.f);
        test = 1;
    }
    ...
    if (g_bitFlag & FLAGMETALNESS)
    {
        if (test) //올리버 머리 임시로
        {
            vSpecular = g_MetalnessTexture.Sample(LinearSampler, In.vTexcoord);
            vPBR = float4(0.f, 0.3, 1.f, 1.f);
